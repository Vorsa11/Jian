...
function getSavedProgress(bookId: string): { chapter: number; lineInChapter: number } {
  try {
    const saved = localStorage.getItem(`reader-progress-v3-${bookId}`);
    if (saved) {
...
function saveProgress(bookId: string, chapter: number, lineInChapter: number) {
  try {
    localStorage.setItem(`reader-progress-v3-${bookId}`, JSON.stringify({ chapter, lineInChapter }));
  } catch {}
}
...
export function TextReader({ content, title, bookId, onClose }: TextReaderProps) {
  ...
  // Sync scroll position when chapter/line changes in scroll mode
  useEffect(() => {
  ...
  }, [currentChapter, lineInChapter, settings.pageMode, settings.fontSize, settings.lineHeight, isAutoReading]);
  ...
  const goToNext = () => {
    if (settings.pageMode === 'page') {
      ...
      if (nextLine >= totalLinesInChapter && currentChapter < chapters.length - 1) {
        // Go to next chapter with animation
      ...
      // Scroll mode: scroll down by viewport height, then snap to start
      if (containerRef.current) {
        const scrollAmount = containerRef.current.clientHeight * 0.85;
        const targetScroll = containerRef.current.scrollTop + scrollAmount;
        
        // Find which chapter we're targeting
        for (let i = 0; i < chapters.length; i++) {
          const chapterEl = chapterRefs.current[i];
          if (!chapterEl) continue;
          ...
          if (targetScroll >= chapterTop && targetScroll < chapterBottom) {
            // Snap to the start of the page within this chapter
            const lineHeightPx = settings.fontSize * settings.lineHeight;
            const linesScrolled = Math.floor((targetScroll - chapterTop) / lineHeightPx);
  ...
  const goToPrev = () => {
    if (settings.pageMode === 'page') {
      ...
      if (prevLine < 0 && currentChapter > 0) {
        // Go to previous chapter with animation
        setIsTransitioning(true);
        setTimeout(() => {
      ...
      // Scroll mode: scroll up by viewport height, then snap to start
      if (containerRef.current) {
        const scrollAmount = containerRef.current.clientHeight * 0.85;
        const targetScroll = Math.max(0, containerRef.current.scrollTop - scrollAmount);
  ...
  if (isImmersive) {
    return (
      ...
      >
        ...
        {/* Chapters Sheet for immersive mode - opaque */}
