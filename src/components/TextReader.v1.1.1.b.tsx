...
export function TextReader({ content, title, bookId, onClose }: TextReaderProps) {
  ...
  const lastImmersiveChapter = useRef(0);
  ...
  useEffect(() => {
    ...
    if (saved.chapter >= 0 && saved.chapter < chapters.length) {
      ...
      lastImmersiveChapter.current = saved.chapter;
  ...
  // Save current chapter when entering immersive mode
  useEffect(() => {
    if (isImmersive) {
      lastImmersiveChapter.current = currentChapter;
    }
  }, [isImmersive, currentChapter]);
  ...
  const goToNext = () => {
    if (settings.pageMode === 'page') {
      ...
      // Scroll mode: scroll down by viewport height (no chapter jump)
      if (containerRef.current) {
  ...
  const goToPrev = () => {
    if (settings.pageMode === 'page') {
      ...
      // Scroll mode: scroll up by viewport height (no chapter jump)
      if (containerRef.current) {
        containerRef.current.scrollTop -= containerRef.current.clientHeight * 0.85;
      }
    }
  ...
  const goToChapter = (index: number) => {
    ...
    lastImmersiveChapter.current = index;
    setLineInChapter(0);
  ...
  const toggleImmersive = () => {
    ...
    if (newImmersive) {
      setShowHeader(false);
      // Restore last chapter position when entering immersive
      if (lastImmersiveChapter.current !== currentChapter && lastImmersiveChapter.current < chapters.length) {
        setCurrentChapter(lastImmersiveChapter.current);
        setLineInChapter(0);
      }
      ...
      setShowHeader(true);
      // Save current chapter when exiting
      lastImmersiveChapter.current = currentChapter;
      if (document.fullscreenElement) {
        ...
        <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
          ...
          {currentChapter > 0 && (
            <button
              ...
              style={{ backgroundColor: `${currentTheme.text}20`, color: currentTheme.text }}
              title="上一章"
            >
          ...
          <button
            ...
            className="h-8 px-3 rounded-full text-xs font-medium transition-all duration-200 hover:scale-105 flex items-center gap-1.5"
            style={{ backgroundColor: `${currentTheme.text}25`, color: currentTheme.text }}
          >
            <List className="h-3.5 w-3.5" />
            <span className="max-w-[100px] truncate">{currentChapterData?.title.slice(0, 12) || '无章节'}</span>
          </button>
          ...
          <button
            ...
            className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium transition-all duration-200 hover:scale-110 ml-1"
            style={{ backgroundColor: `${currentTheme.text}25`, color: currentTheme.text }}
            title="退出沉浸"
          >
            出
