...
function getSavedProgress(bookId: string): { chapter: number; lineInChapter: number } {
  try {
    const saved = localStorage.getItem(`reader-progress-v4-${bookId}`);
    if (saved) {
...
function saveProgress(bookId: string, chapter: number, lineInChapter: number) {
  try {
    localStorage.setItem(`reader-progress-v4-${bookId}`, JSON.stringify({ chapter, lineInChapter }));
  } catch {}
}
...
export function TextReader({ content, title, bookId, onClose }: TextReaderProps) {
  ...
  }, [isAutoReading, autoReadSpeed, settings.pageMode, lineInChapter]);
  ...
  // Update chapter from scroll position (scroll mode only, passive)
  const handleScroll = () => {
    if (settings.pageMode !== 'scroll' || !containerRef.current) return;
    ...
    for (let i = 0; i < chapters.length; i++) {
      ...
      if (scrollTop >= chapterTop - 50 && scrollTop < chapterBottom) {
        if (currentChapter !== i) {
          setCurrentChapter(i);
        }
        // Calculate line position within chapter
        const lineHeightPx = settings.fontSize * settings.lineHeight;
        const relativeScroll = scrollTop - chapterTop;
        const lineInCh = Math.max(0, Math.floor(relativeScroll / lineHeightPx));
        const maxLine = chapters[i].lines.length - 1;
        const clampedLine = Math.min(lineInCh, maxLine);
    ...
    // Clear manual scroll flag after scroll stops
    scrollTimeout.current = setTimeout(() => {
      isManualScroll.current = false;
    }, 150);
  };

  const goToLineInChapter = (line: number, smooth = false) => {
    ...
    if (settings.pageMode === 'scroll' && chapterRefs.current[currentChapter]) {
      ...
      if (chapterEl && containerRef.current) {
        const targetScroll = chapterEl.offsetTop + clampedLine * lineHeightPx;
        if (smooth) {
  ...
  const goToNext = () => {
    if (settings.pageMode === 'page') {
      ...
      // Scroll mode: scroll down by viewport height, then snap to page start
      if (containerRef.current) {
        const scrollAmount = containerRef.current.clientHeight * 0.9;
        const currentScroll = containerRef.current.scrollTop;
        const targetScroll = currentScroll + scrollAmount;
        
        // Find which chapter and page we're targeting
        const lineHeightPx = settings.fontSize * settings.lineHeight;
        ...
        for (let i = 0; i < chapters.length; i++) {
          ...
          if (targetScroll >= chapterTop && targetScroll < chapterBottom) {
            // Calculate which page within this chapter
            const relativeScroll = targetScroll - chapterTop;
            const linesScrolled = Math.floor(relativeScroll / lineHeightPx);
            const pageStartLine = Math.floor(linesScrolled / linesPerPage) * linesPerPage;
            const snapScroll = chapterTop + pageStartLine * lineHeightPx;
            
            containerRef.current.scrollTo({ top: snapScroll, behavior: 'smooth' });
            
            // Update state after scroll
            setTimeout(() => {
              setCurrentChapter(i);
              setLineInChapter(pageStartLine);
            }, 300);
            break;
          }
        }
  ...
  const goToPrev = () => {
    if (settings.pageMode === 'page') {
      ...
      // Scroll mode: scroll up by viewport height, then snap to page start
      if (containerRef.current) {
        const scrollAmount = containerRef.current.clientHeight * 0.9;
        const currentScroll = containerRef.current.scrollTop;
        ...
        for (let i = chapters.length - 1; i >= 0; i--) {
          ...
          if (targetScroll >= chapterTop - 50 && targetScroll < chapterBottom) {
            // Calculate which page within this chapter
            const relativeScroll = Math.max(0, targetScroll - chapterTop);
            const linesScrolled = Math.floor(relativeScroll / lineHeightPx);
            const pageStartLine = Math.floor(linesScrolled / linesPerPage) * linesPerPage;
            const snapScroll = chapterTop + pageStartLine * lineHeightPx;
  ...
  const toggleImmersive = () => {
    ...
    if (newImmersive) {
      setShowHeader(false);
      // Scroll to current position when entering immersive
      if (settings.pageMode === 'scroll' && chapterRefs.current[currentChapter] && containerRef.current) {
        const lineHeightPx = settings.fontSize * settings.lineHeight;
        const chapterEl = chapterRefs.current[currentChapter];
        if (chapterEl) {
          setTimeout(() => {
            containerRef.current!.scrollTop = chapterEl.offsetTop + lineInChapter * lineHeightPx;
          }, 50);
        }
      }
      ...
      setShowHeader(true);
      // Keep current position when exiting immersive
      if (document.fullscreenElement) {
  ...
  if (isImmersive) {
    return (
      ...
      >
        ...
        {/* Top left: Chapter selector button */}
        <button
        ...
        {/* Top right: Chapter navigation + Exit */}
        ...
        <Sheet open={showChapters} onOpenChange={setShowChapters}>
          <SheetContent side="left" className="w-full sm:max-w-md overflow-y-auto p-0 border-r-0" style={{ backgroundColor: currentTheme.bg }}>
            <SheetHeader className="px-4 py-4 border-b" style={{ borderColor: `${currentTheme.text}15` }}>
              <SheetTitle style={{ color: currentTheme.text, fontSize: '1.1rem' }}>目录</SheetTitle>
            </SheetHeader>
