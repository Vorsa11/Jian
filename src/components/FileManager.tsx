      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
        ...
        {!isConfigured ? (
          <div className="space-y-2">
            ...
            <button onClick={saveGithubConfig} className="w-full py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-sm font-medium">点击配置 GitHub</button>
          </div>
        ) : (
          <div className="space-y-2">
            <p className="text-sm text-amber-800">状态：<span className="font-bold text-green-600">已配置</span> ({githubConfig.user})</p>
            <button onClick={saveGithubConfig} className="text-xs text-amber-700 underline hover:text-amber-900">修改配置</button>
          </div>
        )}
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
        ...
        {!user ? (
          <div className="space-y-2">
            ...
            <button onClick={login} className="w-full py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-medium">登录 / 注册</button>
          </div>
        ) : (
          <div className="space-y-2">
            <p className="text-sm text-blue-800">已登录：<span className="font-bold">{user.email}</span></p>
            <button onClick={logout} className="text-xs text-blue-700 underline hover:text-blue-900">切换账号</button>
          </div>
        )}
      </div>
      ...
      {user && (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => loadFiles(user.id)} className="text-xs text-primary hover:underline">刷新</button>
          </div>
          
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {files.length === 0 ? (
              <p className="text-sm text-gray-500 text-center py-4">还没有文件，请上传第一个文件！</p>
            ) : (
              files.map(file => (
